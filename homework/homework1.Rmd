...
> Added lab*_files to .gitignore, to ignore images
...
```
commit 63369ca550e633d6a60542b96be0718dad9183e4 (HEAD -> main)
Author: Qianxi Feng <qianxif@umich.edu>
Date:   Mon Sep 26 00:14:14 2022 -0400
...
> The commit id "8e562" was deleted
...
```
commit 238c2ad663df5412a0169fe23dd4b3bd7ee3b9ff (HEAD -> main)
Author: Qianxi Feng <qianxif@umich.edu>
Date:   Mon Sep 26 00:35:18 2022 -0400

    time3

commit 8e562f1bafa7711b1931feb6fb1b1a36ec7b72ea
Author: Qianxi Feng <qianxif@umich.edu>
Date:   Mon Sep 26 00:31:05 2022 -0400
...
> LICENSE                         data                            labs                            stats306_fall_2022.Rproj
README.md                       homework                        lectures
...
```{r}
displ_col <- mpg$displ
mean <- sum(displ_col) / length(displ_col)
mean
```
```{r}
variance <- sum((displ_col-mean)^2)/length(displ_col-1)
variance
```
...
```{r}
view(starwars)
#The 'NA' means there does not have data shows
```

* How many characters are from Tatooine?
```{r}
length(which(starwars$homeworld == 'Tatooine'))
```

* For humans, what is the average (mean) age?
```{r}
df <- subset(starwars, species == 'Human', na.rm = TRUE)
mean(df$birth_year, na.rm = TRUE)
```
...
# Jabba Desilijic Tiure jums out from the plot
```
...
```{r}
new_starwars <- filter(starwars, mass < 500)
ggplot(data = new_starwars,mapping = aes(x=height,y=mass)) + geom_point() + geom_smooth(method = lm)

# The results have the positive linear relationship with height and mass
```
...
```{r}
ggplot(starwars, aes(homeworld_other)) + geom_bar()
```
...
the results.
```{r}
ggplot(starwars,aes(x=sex,y=height)) + geom_point() + geom_boxplot()

# In the boxplot, the graph shows the relationship between the height and sex. The result told us that male's average height is higher than female.
```
...
```{r}
# I think 
ggplot(data = txhousing, aes(x = factor(month),y = median,color = month)) + geom_point() + coord_polar()
```
...
```{r}
ggplot(txhousing, aes(x = date, y = median, color = city)) + geom_point() + theme(legend.position = "none") + facet_wrap(~city)

# I think group the plot is a better way to display these data. In this graph, Austin and Fort Bend increased biggest in median price during this period. In the given year of 2010,the South Padre Island have the largest variation in prices. 
```
...
median.
```{r}
ggplot(txhousing,aes(x=listings,y=city)) + stat_summary(fun.min="min",fun.max="max",fun=median)
